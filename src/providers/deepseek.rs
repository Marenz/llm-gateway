...
use crate::config::DeepSeekProviderConfig;
...
impl DeepSeekProvider {
    ...
    pub async fn send_json<T: Serialize>(
        ...
        request
            .send()
            .await
            .with_context(|| format!("deepseek request to {} failed", self.name))
    }
    ...
    pub async fn fetch_models(&self) -> anyhow::Result<Vec<serde_json::Value>> {
        ...
        let resp = request
            .send()
            .await
            .with_context(|| format!("deepseek GET /models failed for {}", self.name))?;
                ...
                arr.iter()
                    .filter_map(|m| {
                        ...
                        Some(serde_json::json!({
                            ...
                            "object": "model",
                            "owned_by": m.get("owned_by").cloned().unwrap_or(serde_json::json!("deepseek")),
                        }))
                    })
                    .collect()
            })
